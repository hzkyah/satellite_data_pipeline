...
def run():
   import os
   parser = argparse.ArgumentParser(description='Compute monthly cloud cover')
   ...
   parser.add_argument('--output_file', default='output.txt', help='default=output.txt Supply a location on S3')
   parser.add_argument('--output_dir', required=True, help='Where should the images be stored? Supply an S3 location')
   ...
   # for each month and spacecraft-coverage-pattern (given by the path and row), find all scene
   scenes = (allscenes
